...
flowline_table <- 
  archive::archive_extract("nhdplusv2/Hydrography/NHDFlowline.zip", dir="nhdplusv2/Hydrography", file="NHDFlowline.dbf") |>
  foreign::read.dbf() |> 
  janitor::clean_names() |>
...
flowline_vaattr <- 
  foreign::read.dbf("nhdplusv2/NHDPlusAttributes/PlusFlowlineVAA.dbf") |> 
  as_tibble() |> 
  select(comid = ComID, 
         hydro_seq = Hydroseq,
...
flowline_slopes <- 
  foreign::read.dbf("nhdplusv2/NHDPlusAttributes/elevslope.dbf") |> 
  as_tibble() |> 
...
```{r message=FALSE, warning=FALSE}
archive::archive_extract("nhdplusv2/VogelExtension.zip", dir="nhdplusv2/VogelExtension")

# vogel method mean annual flow and mean annual velocity
vogel_flow <- 
  foreign::read.dbf("nhdplusv2/VogelExtension/vogelflow.dbf") |> 
  as_tibble() |>
  janitor::clean_names() |>
  mutate(across(maflowv:mavelv, function(x) if_else(x>=0, x, NA))) |>
  select(comid, vogel_q_ma_cfs = maflowv, vogel_v_ma_fps = mavelv)

archive::archive_extract("nhdplusv2/VPUAttributeExtension.zip", dir="nhdplusv2/VPUAttributeExtension")

  # incremental precipitation at reach
loc_precip_annual <- 
  read_delim("nhdplusv2/VPUAttributeExtension/IncrPrecipMA.txt") |>
...
import_loc_precip <- function(m) {
  ...
  read_delim(paste0("nhdplusv2/VPUAttributeExtension/IncrPrecipMM", mm, ".txt")) |>
    mutate(month = m) |>
    mutate(comid = as.numeric(FeatureID), loc_ppt_mean_mm = PrecipV/100) |>
...
precip_annual <- 
  read_delim("nhdplusv2/VPUAttributeExtension/CumTotPrecipMA.txt") |>
...
archive::archive_extract("nhdplusv2/EROMExtension.zip", dir="nhdplusv2/EROMExtension")
erom_annual <- 
  foreign::read.dbf("nhdplusv2/EROMExtension/EROM_MA0001.DBF") |>
  as_tibble() |>
...
import_erom <- function(m) {
  mm <- str_pad(m, width=2, pad="0")
  foreign::read.dbf(paste0("nhdplusv2/EROMExtension/EROM_", mm, "0001.dbf")) |>
    as_tibble() |>
    mutate(month = m) |>
