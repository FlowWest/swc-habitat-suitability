...
vb1 <- 
flowlines |> filter(huc_8 %in% selected_huc_8) |> head(2) |> #filter(comid==8062593) |>
  ...
  mutate(vb_width_simple = map(valley_bottom_geoms, possibly(function(x) valley_width(x), otherwise=NA))) |> 
  unnest(vb_width_simple) |>
  mutate(vb_width_transect = map2(valley_bottom_geoms, flowline, function(x, y) valley_width_via_transects(x, y, 1000))) |> 
  unnest(vb_width_transect) |>
