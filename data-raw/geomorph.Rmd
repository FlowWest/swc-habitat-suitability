...
```{r soils-data}
soils <- 
  st_read("/vsizip/soils/Cal_STATSGO2.shp.zip", as_tibble=T) |>
  st_transform(project_crs) |>
  janitor::clean_names() |> 
...
geomorph_site_data <- 
  tibble(result=rjson::fromJSON(file="ucd_geomorph/geoSites.json")) |> 
  unnest_wider(result) |> 
  ...
  inner_join(read_csv("ucd_geomorph/geomorph_site_attributes.csv"), by=join_by(identity)) |>
  st_transform(project_crs) |> 
...
geomorph_pred |> select(comid, geomorph_class) |> saveRDS("../data/attr_geomorph_class.Rds")
