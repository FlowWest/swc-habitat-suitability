...
stan_calc_hsi <- function(g, q){
  stan_thiessen |>
    ...
    summarize(area = units::drop_units(sum(area)),
              wua_simp = sum(hsi_simp),
              wua_frac = sum(hsi_frac)) |>
    mutate(pcthab_simp = wua_simp / area,
           pcthab_frac = wua_frac / area) |>
    st_drop_geometry()
}

stan_hsi <- 
  stan_comid |>
  expand_grid(discharge_cfs = names(stan_result_filenames)) |>
