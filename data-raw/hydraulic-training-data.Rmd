...
durhsi_by_first_inundating_flow <- readRDS("../data/durhsi_by_model_q.Rds")
...
stan_vid_lowland <- stan_thiessen |>
  mutate(lowland = st_intersects(geometry, st_transform(valley_lowland, st_crs(stan_thiessen)))) |>
  select(vid, lowland)
...
stan_durhsi_by_vid <- stan_result |>
  ...
  left_join(stan_vid_lowland, by=join_by(vid), relationship="one-to-one") |>
...
stan_calc_hsi <- function(g, q){
  ...
  stan_thiessen |>
    ...
    mutate(area_m2 = if_else(depth_ft>0, units::drop_units(st_area(geometry)), 0),
           ...
           wua_sjrm = hsi_sjrm * area_m2,
           wua_shqt = hsi_shqt * area_m2,
           wua_simp_dur = hsi_simp * area_m2 * avg_durhsi,
           wua_frac_dur = hsi_frac * area_m2 * avg_durhsi,
           wua_sjrm_dur = hsi_sjrm * area_m2 * avg_durhsi,
           wua_shqt_dur = hsi_shqt * area_m2 * avg_durhsi) |>
...
raster_hsi <- function(g, q, crs="EPSG:6416"){
  ...
  area_wua_d <- terra::global(hsi_d, "sum", na.rm=TRUE)[[1]]
  area_pct_d <- area_wua_d / area_tot
  area_wua_hqt_d <- terra::global(hsi_hqt_d, "sum", na.rm=TRUE)[[1]]
  area_pct_hqt_d <- area_wua_hqt_d / area_tot_hqt
...
yuba_vid_lowland <- yuba_thiessen |>
  mutate(lowland = st_intersects(geometry, st_transform(valley_lowland, st_crs(yuba_thiessen)))) |>
  select(vid, lowland)
...
yuba_durhsi_by_vid <- yuba_result |>
  ...
  left_join(durhsi_by_first_inundating_flow |> 
            ...
            by=join_by(first_inundating_flow == model_q),
            relationship="many-to-one") |>
  left_join(yuba_vid_lowland, by=join_by(vid), relationship="one-to-one") |>
  mutate(avg_durhsi = if_else(lowland, avg_durhsi_vl, avg_durhsi_vf)) |>
  select(vid, first_inundating_flow, avg_durhsi) |>
  glimpse()
```
...
yuba_calc_comid_hsi <- function(g, q, t){
  ...
  t |>
    ...
    left_join(yuba_durhsi_by_vid, by=join_by(vid)) |>
    mutate(avg_durhsi = coalesce(avg_durhsi, 0)) |>
    st_intersection(g) |>
    mutate(area_ft2 = if_else(depth_ft>0, units::drop_units(st_area(geometry)), 0),
           ...
           wua_simp_dur = hsi_simp * area_ft2 * avg_durhsi,
           wua_frac_dur = hsi_frac * area_ft2 * avg_durhsi,
           wua_sjrm_dur = hsi_sjrm * area_ft2 * avg_durhsi,
           wua_shqt_dur = hsi_shqt * area_ft2 * avg_durhsi) |>
...
fsa_combined |> saveRDS("../data/fsa_combined.Rds")
```
