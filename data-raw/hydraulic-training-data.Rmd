...
stan_baseflow_channel <- stan_thiessen |>
...
# # bankfull channel is the boundary between in-stream and floodplain
# stan_bankfull_channel <- stan_thiessen |>
#   filter(vid %in% stan_result |> filter(inundated & discharge_cfs==1250) |> pull(vid)) |>
#   summarize() |>
#   st_union()
```
...
stan_calc_hsi <- function(g, q){
  ...
  stan_thiessen |>
    ...
    mutate(bfc_mask = if_else(bfc, 0, 1),
...
yuba_durhsi_by_vid <- function(t) {

  # first determine which vids are valley lowland vs foothill
  yuba_vid_lowland <- t |>
    mutate(lowland = st_intersects(geometry, st_transform(valley_lowland, st_crs(t)))) |>
    select(vid, lowland)
...
yuba_calc_comid_hsi <- function(g, q, t){
  ...
  t |>
    ...
    left_join(yuba_durhsi_by_vid(t), by=join_by(vid)) |>
    mutate(avg_durhsi = coalesce(avg_durhsi, 0)) |>
    st_intersection(g) |>
    st_join(yuba_bfc) |>
    mutate(bfc_mask = if_else(bfc, 0, 1),
