...
training_cdec_data |> 
  ...
  geom_point(aes(x = date, y = q_gauge), size=1) 
...
calculate_exceedence <- function(q_gauge, q_crit) {
  ...
  exceedence_intervals <- 
    ...
    mutate(durhsi_vl = case_when(duration_days == 0  ~ 0,
                                 duration_days < 18  ~ 2/3,
                                 duration_days < 25 ~ 1.00,
                                 duration_days >= 25 ~ 2/3),
           durhsi_vf = case_when(duration_days < 0  ~ 0,
                                 duration_days < 10 ~ 1,
                                 duration_days >= 10 ~ 2/3)) 
...
# example usage
calculate_exceedence(filter(training_cdec_data, river=="Deer Creek" & water_year==2017)$q_gauge, 500)
...
durhsi_by_model_q |> ggplot() +
  ...
  #geom_ribbon(aes(x = model_q, 
  #                ymin = pmax(avg_days_inundated - avg_days_inundated_sdev,0), 
  #                ymax = avg_days_inundated + avg_days_inundated_sdev)) +
  #geom_errorbar(aes(x = model_q, 
  #                  ymin = pmax(avg_days_inundated - avg_days_inundated_sdev,0), 
  #                  ymax = avg_days_inundated + avg_days_inundated_sdev)) +
  geom_line(aes(x = model_q, y = avg_days_inundated)) + 
  #geom_text(aes(x = model_q, y = avg_days_inundated, label = n_inundations), size=2) +
...
durhsi_by_model_q_wy |> saveRDS("../data/durhsi_by_model_q_wy.Rds")
```

*Alternate method to apply this output:*
...
fractional_range_sum <- function(a, b, ind, val) {
  if(b==0){
    ...
    # if range is entirely below the first data element, just draw from that first element
    return(linterp(b, 0, ind[[1]], 0, val[[1]]) - linterp(a, 0, ind[[1]], 0, val[[1]]))
  } else {
    v1 <- val[which(ind<=a)] 
    v2 <- val[which(ind>a & ind<b)]
    v3 <- val[which(ind>=b)]
    i1 <- ind[which(ind<=a)]
    i2 <- ind[which(ind>a & ind<b)]
    i3 <- ind[which(ind>=b)]
...
duration_rating_curves <- durhsi_by_model_q |> 
  ...
  mutate(durhsi_vl = case_when(avg_days_inundated == 0  ~ 0,
                               avg_days_inundated < 18  ~ 2/3,
                               avg_days_inundated < 25 ~ 1.00,
                               avg_days_inundated >= 25 ~ 2/3),
         durhsi_vf = case_when(avg_days_inundated < 0  ~ 0,
                               avg_days_inundated < 10 ~ 1,
                               avg_days_inundated >= 10 ~ 2/3)) 
