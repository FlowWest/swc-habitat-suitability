...
water_year_types <- read_csv("water_year_type/water_year_type.csv") |>
...
calculate_exceedence_summary <- function(q_gauge, q_crit) {
...
durhsi_by_model_q_wy <- 
  ...
  mutate(durhsi_vl = case_when(max_days_inundated == 0  ~ 0,
                               max_days_inundated < 18  ~ 2/3,
                               max_days_inundated < 25 ~ 1.00,
                               max_days_inundated >= 25 ~ 2/3),
         durhsi_vf = case_when(max_days_inundated <= 0  ~ 0,
                               max_days_inundated < 10 ~ 1,
                               max_days_inundated >= 10 ~ 2/3)) |> 
  glimpse()
...
```{r posthoc-duration-func, eval=FALSE, include=TRUE}
# full version
...
fsa_duration_posthoc <- inner_join(fsa_combined |> nest(.by=c(dataset, comid), .key="tbl_fsa"),
...
```{r daily-flow-exceedence-functions, message=FALSE, warning=FALSE}
...
fsa <- fsa_combined |>
  ...
  mutate(wua_per_lf = area_wua_ft2_hqt / (reach_length_km*3280.84)) |>
...
drc |> ggplot() + geom_line(aes(x=model_q, y=dhsi)) + scale_x_log10() + scale_y_continuous(limits=c(0,1)) +
...
res |> 
  ...
  geom_line(aes(y=dhsi*20, color="DHSI")) +
  ggtitle("Duration Criteria Applied to Flow-to-Suitable-Area Curve") +
