...
si_pred <- 
  ...
  transmute(comid, flow_norm_cfs=exp(flow_norm_cfs), hsi_frac_pred) |>
...
flowlines |> st_zm() |>
  ...
  ggplot() + geom_sf(aes(color = wua_per_lf_pred*1000)) + 
...
mainstems <- 
  ...
  filter(!(str_detect(river, "Sacramento") | str_detect(river, "San Joaquin")))
...
pd_dsmhabitat_pred <- 
  ...
  prep(td) |> 
  bake(pd_dsmhabitat) |> 
  bind_cols(pd_dsmhabitat |> select(comid)) |>
  drop_na() %>% # need to use magrittr pipe for this purpose
  mutate(log_wua_per_lf_pred = predict(rfr_sd$fit$fit, new_data=.)[[".pred"]]) |>
  transmute(comid, flow_cfs=exp(flow_cfs), log_wua_per_lf_pred) |>
  mutate(wua_per_lf_pred = exp(log_wua_per_lf_pred)) |>
