...
```{r svr-si2, eval=FALSE, message=FALSE, include=FALSE}
rfr_spec <- svm_rbf(mode = "regression")
...
try imposing a momnotoincity constraint on all variables *EXCEPT* flow_cfs

```{r btr-si2, eval=FALSE, message=FALSE, include=FALSE}
si2_rec$var_info |>
  mutate(monotonicity = case_when(
    # no monotonicity constraint
    variable == flow_norm_cfs ~ 0,
    # positive constraint
    variable %in% c() ~ 1,
    # negative constraint
    variable %in% c() ~ -1,
...
rfr_spec <- boost_tree(mode = "regression") |>
  set_engine(engine = "xgboost", 
             params = list(monotone_constraints = 1 * ((si2_rec$var_info$role=="predictor") & (si2_rec$var_info$variable!="flow_norm_cfs"))))
