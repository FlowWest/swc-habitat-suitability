...
lm_si_res |> 
  ...
  coord_fixed() + geom_abline() + scale_color_viridis_c(trans="log10") + 
...
lm_sd_res |>
  ...
  scale_y_log10() + scale_x_log10() +  coord_fixed() + geom_abline() + scale_color_viridis_c(trans="log10") + 
...
```{r map-output-sd}
flowlines |> st_zm() |>
  inner_join(sd_pred |> filter(as.integer(flow_cfs)==800), by=join_by(comid)) |>
