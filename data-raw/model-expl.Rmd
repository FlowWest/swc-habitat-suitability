...
```{r prediction-output-sd-fig, eval=FALSE, fig.height=6.5, fig.width=6.5, dpi=300, include=FALSE}
n_breaks <- 10
sd_pred |>
  inner_join(flowline_attributes, by=join_by(comid), relationship="many-to-one") |>
  select(comid, flow_cfs, wua_per_lf_pred, 
         da_avg_slope, da_elev_mean, da_k_erodibility,
         slope, sinuosity, bf_w_d_ratio, 
         vb_bf_w_ratio, loc_permeability, bf_w_d_ratio, mtpi30_min, ) |>
  mutate(across(-(1:3), function(x) cut(x,breaks=n_breaks, labels=seq(1,n_breaks)))) |>
  pivot_longer(cols = -(1:3)) |>
  ggplot(aes(x = flow_cfs, y = wua_per_lf_pred)) +
  facet_wrap(~name) + geom_smooth(aes(color = value)) +
