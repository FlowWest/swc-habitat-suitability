...
# TEST: Optionally, calculate the HSI fraction as the WUA per bankfull area, rather than WUA per inundated area
# Then the division can be reliably reversed post-prediction
if(FALSE){
  flow_to_suitable_area <-
    flow_to_suitable_area |>
    inner_join(flowline_attributes |> transmute(comid,
                                                chan_width_ft = chan_width_m / 0.3048, 
                                                reach_length_ft = reach_length_km * 3280.84)) |>
    mutate(active_channel_area_ft2 = reach_length_ft * chan_width_ft,
           hsi_frac = area_wua_ft2 / active_channel_area_ft2) 
}
```
...
**To actually apply these results, we need to also have an estimate of total inundated area, which can be then multiplied with the predicted %HSI.**
...
A potential compromise between the previous two models that scales the results by mean annual flow, then unscales them after prediction.
...
```{r dsmhabitat-sd-watershed-comps}
pd_rf_by_mainstem |> 
