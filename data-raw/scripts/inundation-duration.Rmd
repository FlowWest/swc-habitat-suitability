...
Duration suitability for spawning (based on average incubation time of 12 weeks):
...
training_reach_gages <- tribble(
  ...
  mutate(filename = pmap_chr(list(cdec_sta, cdec_sen, cdec_dur), 
                             function(x, y, z) cdec_csv_retrieve(sta=x, sen=y, dur=z))) |>
...
calculate_exceedence_summary <- function(q_gauge, q_crit, baseflow_threshold_days = 100) {
  ...
  exceedence_intervals <- 
    ...
    mutate(#duration_days = pmin(duration_days, 365), 
           durhsi_vl = habistat::duration_hsi_rearing_vl(duration_days, baseflow_threshold_days),
           durhsi_vf = habistat::duration_hsi_rearing_vf(duration_days, baseflow_threshold_days)) 
...
# }
# 
# update: moved to the habistat package
# example usage
duration_calc_days_inundated(filter(training_cdec_data, river=="Deer Creek" & water_year==2017)$q_gauge, 500)
```
...
durhsi_by_model_q_wy <- 
  ...
  inner_join(water_year_types, by=join_by(location, water_year), relationship="many-to-one") |>
    mutate(durhsi_vl = habistat::duration_hsi_rearing_vl(max_days_inundated, baseflow_threshold_days),
           durhsi_vf = habistat::duration_hsi_rearing_vf(max_days_inundated, baseflow_threshold_days)) |>
  glimpse()
...
duration_rating_curves <- durhsi_by_model_q |> 
  ...
  mutate(durhsi_vl = habistat::duration_hsi_rearing_vl(avg_days_inundated, baseflow_threshold_days),
         durhsi_vf = habistat::duration_hsi_rearing_vf(avg_days_inundated, baseflow_threshold_days)) 
...
dhsi <- function(duration_days, type) {
  if (type=="vl") {
    habistat::duration_hsi_rearing_vl(duration_days, baseflow_threshold_days)
  } else if (type=="vf") {
    habistat::duration_hsi_rearing_vf(duration_days, baseflow_threshold_days)
  }
}
...
# apply_durations_to_fsa_curve <- possibly(apply_durations_to_fsa_curve, otherwise=NA)
# Functions were moved to the habistat package
```
...
res <- duration_apply_dhsi_to_fsa_curve(fsa, drc, 
                                    # specify variable names
...
batch_res <-
  ...
  mutate(result = map2(tbl_fsa, tbl_drc, 
                       function(f, d) duration_apply_dhsi_to_fsa_curve(f, d, 
                       # specify variable names
                       fsa_q = flow_cfs, fsa_wua = wua,
                       drc_q = model_q, drc_dhsi = dhsi))) |>
...
test_output <- fsa |> 
  flow_threshold_remove_baseflow(test_threshold, .flow_var=flow_cfs, .habitat_var=wua) |>
  glimpse()
...
## Floodplain-Instream Split (test/ not run)

Need method to determine the flow threshold (aka bankfull flow) to use to split instream and floodplain
...
```{r floodplain-split-func, eval=FALSE, include=FALSE}
...
```{r floodplain-split-example, eval=FALSE, include=FALSE}
