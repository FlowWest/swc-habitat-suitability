...
#  mutate(da_scalar_maf = (da_area_sq_km*247.1053815) * (da_ppt_mean_mm*0.0393701) / 1E6) #|># 1 million acre-inches
...
wua_hydraulic <-
  list("Lower Yuba River" = readRDS(here::here("data-raw", "results", "fsa_yuba.Rds")),
       "Stanislaus River" = readRDS(here::here("data-raw", "results", "fsa_stanislaus.Rds")),
       "Deer Creek" = readRDS(here::here("data-raw", "results", "fsa_deer.Rds")),
       "Tuolumne River (Basso-La Grange)" = readRDS(here::here("data-raw", "results", "fsa_basso.Rds"))) |>
...
#   mutate(across(c(area_tot_ft2, area_wua_ft2, hsi_frac, wua_per_lf), function(var) zoo::na.approx(var, x = # flow_cfs, na.rm=F))) |>
...
flow_to_suitable_area <- 
  ...
  complete(flow_cfs = interp_flows) |>
  arrange(dataset, comid, flow_cfs, length_ft) |>
  mutate(across(c(area_tot, area_wua, hsi_frac, wua_per_lf), function(var) zoo::na.approx(var, x = flow_cfs, na.rm=F))) |>
  filter(flow_cfs %in% interp_flows) |>
  filter(!is.na(hsi_frac))
