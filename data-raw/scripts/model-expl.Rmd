...
yuba_dsmhabitat_summary <-
  ...
  pivot_wider() |>
  mutate(total_DSMhabitat = instream_DSMhabitat + floodplain_DSMhabitat)

ggplot() + 
  geom_line(data = yuba_wua_predicted_summary,
            aes(x = flow_cfs, y = wua_per_lf_actual, color = "Habistat Training (LYR SRH2D)", linetype = "instream + floodplain")) +
  geom_line(data = yuba_wua_predicted_summary,
            aes(x = flow_cfs, y = wua_per_lf_pred_sd, color = "Habistat Model 1", linetype = "instream + floodplain")) +
  geom_line(data = yuba_wua_predicted_summary,
            aes(x = flow_cfs, y = wua_per_lf_pred_si2, color = "Habistat Model 2", linetype = "instream + floodplain")) +
  geom_line(data = lyr_cbec_nontemporal_summary,
            aes(x = flow_cfs, y = wua_ft2_per_lf, color = "CBEC Study (LYR SRH2D)", linetype = "instream + floodplain")) +
  geom_line(data = yuba_dsmhabitat_summary,
            aes(x = flow_cfs, y = total_DSMhabitat, color = "DSMhabitat", linetype = "instream + floodplain")) +
  geom_line(data = yuba_dsmhabitat_summary,
            aes(x = flow_cfs, y = instream_DSMhabitat, color = "DSMhabitat", linetype = "instream")) +
  geom_line(data = yuba_dsmhabitat_summary,
            aes(x = flow_cfs, y = floodplain_DSMhabitat, color = "DSMhabitat", linetype = "floodplain")) + 
...
ggplot() + 
  geom_point(data = lyr_cbec_temporal_summary,
            aes(x = flow_cfs, y = wua_ft2_per_lf, color = "CBEC Study (LYR SRH2D)"), size = 0.25, stroke = 0) +
  geom_line(data = lyr_cbec_nontemporal_summary,
  ...
  xlab("Flow (cfs)") + ylab("Suitable habitat area (ft2) per linear ft") +
  theme(panel.grid.minor = element_blank()) + annotation_logticks(sides = "b") +
  scale_linewidth_manual(name = "model type", values = c("Non-temporal" = 1, "Temporal" = 0.5)) +
