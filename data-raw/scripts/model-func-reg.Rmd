...
# forked version fixing compile error
...
```{r}
prep_func_reg <- function(df_x, df_y, ..., # pass predictor variables as tidy eval list
                          .y_var = "wua_per_lf",
                          .arg_var = "flow_cfs",
                          .rep_var = "comid") {
...
lhs <- flow_to_suitable_area |> 
  recipe(wua_per_lf + flow_cfs ~ comid) |> 
...
Training data is now compiled for `r cat(dim(data$y)[[2]])` flows and `r cat(dim(data$x)[[2]])` predictors across `r cat(dim(data$y)[[1]])` comids.

* Response (y) matrix dimensions: `r cat(paste(dim(data$y), collapse=" x "))`
* Predictor (x) matrix dimensions: `r cat(paste(dim(data$x), collapse=" x "))`

### Fitting the model

```{r}
fit <- train_data |>
  fit_func_reg(n.iters=1000, degree=3) |>
  glimpse()
...
Now we have a `r cat(paste(dim(coefs), collapse=" x "))` coefficient matrix (rows are predictors and the intercept, columns are flows) 
...
predict_func_reg <- function(data, fit) {
  ...
  colnames(coefs) <- data$arg_vals
  
  result_mat <- cbind(1, data$x) %*% coefs
  return(list("arg_vals" = data$arg_vals,
...
tidy_func_reg <- function(lst,
                          ...
                          .arg_var = "flow_cfs",
                          .rep_var = "comid") {
  cbind(lst$rep_names, lst$predicted) |>
    as_tibble() |>
