...
interp_flows <- seq_log10(100, 15000, 0.05)
...
model_data <- habistat::flowline_attr |>
  ...
  mutate(across(c(model_variant, model_type, model_name, model_hab), as_factor)) # for storage efficiency
```
...
#    } else if(x == "ML") {
#        mlp(mode = "regression",
#            hidden_units = c(8, 8, 8), # number of nodes in each hidden layer
#            penalty = 0.0, # L1 lasso penalty
#            dropout = 0.1, # both penalty and dropout should not be used
#            epochs = 100,
#            learn_rate = 0.01,
#            activation = "relu") |>
#        set_engine("brulee")
...
#  # FIT MODELS -----------------------------------------------------------------
#  mutate(model_fit = pmap(list(training, model_recipe, model_spec, model_type), 
#                               function(d, rec, spec, typ) {
#    if(typ == "ML") {
#      workflow() |>
#        add_recipe(rec) |>
#        add_model(spec) |>
...
model_result |> saveRDS(here::here("data-raw", "results", "model_result.Rds"))
```
...
scale_color_flowlines <- c("Mainstems" = "darkblue",
                           "Tributaries" = "cornflowerblue")
                           #"Not Included"= "darkgray")
  
plt_rearing <- 
  ggplot() + 
  geom_sf(data=habistat::cv_watersheds, fill = "lightgray", color = NA) + #, color = "darkgray") + 
  geom_sf(data=flowlines_pred, aes(color = "Tributaries")) +
  geom_sf(data=habistat::cv_mainstems, aes(color="Mainstems")) +
  theme(axis.text = element_blank()) +
  ggtitle("Rearing") +
  scale_color_manual(name = "Flowline Type",
                     values = scale_color_flowlines,
                     limits = names(scale_color_flowlines))

plt_spawning <- 
  ggplot() + 
  #geom_sf(data=flowlines_pred, aes(color="Not Included")) +
  geom_sf(data=habistat::cv_watersheds, fill = "lightgray", color = NA) + #, color = "darkgray") + 
  geom_sf(data=flowlines_pred_spawning, aes(color = "Tributaries")) +
  geom_sf(data=habistat::cv_mainstems |> filter(comid %in% flowlines_pred_spawning$comid), aes(color="Mainstems")) +
  theme(axis.text = element_blank()) +
  ggtitle("Spawning") +
  scale_color_manual(name = "Flowline Type",
                     values = scale_color_flowlines,
...
```{r}
inner_join(habistat::wua_hydraulic |> filter(habitat=="rearing" & !bfc) |> select(dataset, comid, flow_cfs, wua_per_lf),
           habistat::wua_predicted |> filter(model_name=="SD" & !model_bfc & habitat=="rearing") |> select(comid, flow_cfs, wua_per_lf_pred),
           by=join_by(comid, flow_cfs)) |>
  ggplot() + geom_point(aes(x = wua_per_lf, y = wua_per_lf_pred, color = flow_cfs, group = comid)) + 
  scale_x_log10(breaks=c(1, 10, 100, 1000)) + 
  scale_y_log10(breaks=c(1, 10, 100, 1000)) + 
  facet_wrap(~dataset) + 
  coord_fixed() + annotation_logticks() +
  theme(panel.grid.minor = element_blank()) + 
  xlab("Actual WUA/LF") + ylab("Predicted WUA/LF") +
  geom_abline(slope = 1, intercept = 0)

v1 <- readRDS(here::here("data-raw", "temp", "backup20240822", "model_predicted_spawning.Rds"))
v2 <- readRDS(here::here("data-raw", "results", "model_predicted_spawning.Rds"))
inner_join(v1 |> filter(model_name=="SD") |> select(comid, flow_cfs, wua_per_lf_pred1 = wua_per_lf_pred),
           v2 |> filter(model_name=="SD") |> select(comid, flow_cfs, wua_per_lf_pred2 = wua_per_lf_pred)) |>
  inner_join(wua_hydraulic_interp |> filter(habitat=="spawning") |> select(comid, flow_cfs, wua_per_lf))
```
