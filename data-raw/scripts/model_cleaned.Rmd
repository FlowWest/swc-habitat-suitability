...
if(!file.exists(here::here("data-raw", "results", "model_result.Rds"))) {
...
model_result <- 
  ...
  mutate(model_formula = map(model_variant, 
    ...
    if(x == "SD0") { # bare version
      ihs_wua_per_lf + case_wt + comid + flow_idx ~ 
    ...
    if(x == "SD2") { # full version
      ...
      loc_bfi + loc_pct_clay + loc_pct_sand + loc_permeability + loc_bedrock_depth + loc_ppt_mean_mm +
      mtpi30_min + vb_bf_w_ratio + frac_leveed_longitudinal + vb_width_transect + divergence_ratio
    } else 
    if(x == "SN0") { # bare version
      ihs_wua_per_lf_norm + case_wt + comid + da_scalar_maf + flow_idx ~ 
    ...
    if(x == "SN2") { # full version
      ...
      da_k_erodibility + da_avg_slope + mean_ndvi +
      loc_bfi + loc_pct_clay + loc_pct_sand + loc_permeability + loc_bedrock_depth + loc_ppt_mean_mm +
      mtpi30_min + vb_bf_w_ratio + frac_leveed_longitudinal + divergence_ratio
    }
  })) |>
  ...
  mutate(model_recipe = pmap(list(training, model_name, model_formula), 
    ...
    if(x == "SD") {
      ...
      update_role(c(comid, flow_idx), new_role = "identifier") |>
...
model_result <- readRDS(here::here("data-raw", "results", "model_result.Rds"))
...
model_val |>
  ...
  ggplot(aes(x = flow_cfs, y = rmse)) + geom_line(aes(linetype = train_test, color = paste(model_hab, model_type))) +
...
# FOR TESTING
row <- model_result |> filter(row_number()==6)
row <- model_result |> filter(row_number()==12)
training_df <- row$training[[1]]
x <- row$model_name[[1]]
rec <- row$model_recipe[[1]]
m <- row$model_fit[[1]]
v <- row$model_variant[[1]]

alpha_ci <- 0.90
alpha_pi <- 0.90

if(!file.exists(here::here("data-raw", "results", "model_predicted.Rds"))) {

model_predicted <-  
  model_result |>
  #filter(model_type=="RF") |>
  #select(model_bfc, model_variant, model_type, model_name, model_recipe, model_fit, training) |>
  mutate(predicted = pmap(list(training, model_name, model_recipe, model_fit, model_variant),
                               function(training_df, x, rec, m, v) {
...
model_predicted <- readRDS(here::here("data-raw", "results", "model_predicted.Rds"))
...
model_predicted_filtered <- 
  ...
  transmute(comid, model_bfc, model_name, flow_idx, flow_cfs, wua_per_lf_pred, 
...
flowlines_pred_spawning <- 
  ...
  filter(ftype %in% c("StreamRiver") & divergence_ratio > 0.5)
...
wua_predicted_cv_watersheds <- 
  ...
  group_by(habitat, model_bfc, model_name, watershed_level_3, flow_idx, flow_cfs) |>
  summarize(wua_per_lf_pred = sum(wua_per_lf_pred * reach_length_ft) / sum(reach_length_ft),
...
wua_predicted_cv_mainstems <- 
  wua_predicted |>
  filter(!is.na(river_cvpia)) |>
  group_by(habitat, model_bfc, model_name, river_cvpia, flow_idx, flow_cfs) |>
  summarize(wua_per_lf_pred = sum(wua_per_lf_pred * reach_length_ft) / sum(reach_length_ft),
            wua_acres_pred = sum(wua_per_lf_pred * reach_length_ft) / 43560) |>
  ungroup()
