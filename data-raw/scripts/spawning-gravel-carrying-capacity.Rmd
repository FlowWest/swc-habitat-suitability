...
battle_redd <- read_csv(here::here("data-raw", "source", "spawning_data", "battle", "jpe_battle_creek_redd_counts.csv")) |> 
  ...
  select(date, longitude, latitude, river_mile, reach, redd_measured, redd_length) |> 
  filter(redd_measured == TRUE) |> 
  glimpse()

battle_redd |> 
  group_by(river_mile) |> 
  summarise(n_redd = length(redd_measured)) |> 
ggplot(aes(x = river_mile, y = n_redd)) +
  geom_col() +
  ggtitle('Redd Counts for Battle Creek by River Mile')

battle_redd_for_agg <- battle_redd |> 
  group_by(river_mile) |> 
  summarise(n_redd = length(redd_measured)) |> 
  mutate(river = "Battle Creek")
```
...
clear_redd |> 
  ...
  group_by(lat_long) |> 
  summarise(n_redd = length(measured)) |> 
ggplot(aes(x = lat_long, y = n_redd)) +
  geom_col() +
  ggtitle('Redd Counts for Clear Creek by lat/long')

clear_redd_for_agg <- clear_redd |> 
  mutate(lat_long = paste0(latitude, ", ", longitude),
         river = "Clear Creek") |> 
  select(-date) |> 
  glimpse()
...
mill_locations <- readxl::read_excel(here::here("data-raw", "source", "spawning_data", "mill", 'adult-holding-redd-and-carcass-surveys_mill-creek_data-raw_Mill Creek spring-run Redd Counts 2007-2023.xlsx'), sheet = "nearest temp-logger") |> 
  janitor::clean_names() |> 
  rename(location = redd_survey_section) |> 
  glimpse()

mill_merge <- mill_redd |> 
  left_join(mill_locations) |> 
  select(-starting_elevation) |> 
  filter(surveyed == TRUE) |> 
  group_by(location, lat, long) |> 
  summarise(n_redds = sum(redd_count)) |> 
