...
naip_mosaic <- 
  ...
  read_stars(along="band", proxy=TRUE)

naip_mosaic |>
  mutate(# add code here for doing naip math and thresholding, keep it all as a proxy!)
...
# threshold to identify vegetated pixels
...
# summarize percent vegetated pixels within channels and valleys

buffers <-
  flowlines |> 
  mutate(channel_buffer = st_buffer(geometry, width = bf_width_m/2)) |>
  st_drop_geometry() |>
  ...
  mutate(valley_bottom = map2(channel_buffer, valley_bottom_geoms, 
                              function(cb, vb) st_union(cb, vb, by_feature = FALSE) |> st_sfc(crs=st_crs(flowlines))))

ndvi_pct <-
  buffers |>
  st_drop_geometry() |>
  select(comid) |>
  mutate(pct_ndvi_cb = terra::zonal(naip_ndvi_cls, buffers$channel_buffer, "mean") |> rename(pct_ndvi_cb = mean),
         pct_ndvi_vb = terra::zonal(naip_ndvi_cls, buffers$valley_bottom, "mean") |> rename(pct_ndvi_vb = mean))

ndvi_pct |> saveRDS("./data/attr_ndvi_pct.Rds")
